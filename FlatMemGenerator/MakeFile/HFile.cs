...
namespace FlatMemGenerator
{
    public class HFile
    {
        public static void Make()
        {
            ...
            hContent.AppendLine();
            hContent.AppendLine(@"#include <Engine/Tx/MemField.h>");
            hContent.AppendLine(@"#include <Engine/Tx/MemObject.h>");
            hContent.AppendLine();
            ...
            foreach (var typeOne in SC.tree.m_types)
            {
                ...
                hContent.AppendLine($"    class {typeOne.m_name}Mem : public T,");
                hContent.AppendLine($"        private {typeOne.m_name}T,");
                hContent.AppendLine($"        std::enable_shared_from_this<{typeOne.m_name}Mem<T>>");
                hContent.AppendLine(@"    {");
                foreach (var field in typeOne.m_fields)
                {
                ...
                hContent.AppendLine($"        {typeOne.m_name}Mem(MemTx& tx);");
                hContent.AppendLine($"        std::shared_ptr<IMemObject> CreateObject() override;");
                hContent.AppendLine();
                foreach (var field in typeOne.m_fields)
                {
                    hContent.AppendLine($"        class {SC.SnakeToPascalOrCamel(field.m_name)} : public MemField");
                    hContent.AppendLine(@"        {");
                    hContent.AppendLine(@"        public:");
                    hContent.AppendLine($"            {SC.SnakeToPascalOrCamel(field.m_name)}({typeOne.m_name}Mem& owner);");
                    ...
                    hContent.AppendLine($"            {typeOne.m_name}Mem& m_owner;");
                ...
                foreach (var field in typeOne.m_fields)
                {
                    ...
                    hContent.AppendLine($"        MemField* m_p{SC.SnakeToPascalOrCamel(field.m_name)} = nullptr;");
                }
                ...
                hContent.AppendLine(@"        };");
                hContent.AppendLine();
                hContent.AppendLine(@"        std::unordered_map<std::string, std::shared_ptr<MemField>> m_fieldMap;");
                hContent.AppendLine(@"        std::vector<const MemField*>                               m_fieldVector;");
                ...
                hContent.AppendLine($"    template<typename T>");
                hContent.AppendLine($"    {typeOne.m_name}Mem<T>::{typeOne.m_name}Mem(MemTx& tx)");
                ...
                hContent.AppendLine($"    template<typename T>");
                hContent.AppendLine($"    std::shared_ptr<IMemObject> {typeOne.m_name}Mem<T>::CreateObject()");
                hContent.AppendLine(@"    {");
                hContent.AppendLine($"        return std::make_shared<{typeOne.m_name}Mem<T>>(this->m_tx);");
                hContent.AppendLine(@"    };");
                hContent.AppendLine();
                ...
                {
                    hContent.AppendLine($"    template<typename T>");
                    hContent.AppendLine($"    {typeOne.m_name}Mem<T>::{SC.SnakeToPascalOrCamel(field.m_name)}::{SC.SnakeToPascalOrCamel(field.m_name)}({typeOne.m_name}Mem& owner)");
                    ...
                    {
                        hContent.AppendLine($"    template<typename T>");
                        hContent.AppendLine($"    void {typeOne.m_name}Mem<T>::{SC.SnakeToPascalOrCamel(field.m_name)}::Set(const {SC.ChangeToCppType(field.m_type)}& param)");
                        hContent.AppendLine(@"    {");
                    ...
                    hContent.AppendLine($"    template<typename T>");
                    hContent.AppendLine($"    void {typeOne.m_name}Mem<T>::{SC.SnakeToPascalOrCamel(field.m_name)}::Set(const boost::mysql::field_view param)");
                    hContent.AppendLine(@"    {");
                    ...
                    hContent.AppendLine($"    template<typename T>");
                    hContent.AppendLine($"    void {typeOne.m_name}Mem<T>::{SC.SnakeToPascalOrCamel(field.m_name)}::SetKey(const {SC.ChangeToCppType(field.m_type)}& param)");
                    hContent.AppendLine(@"    {");
                    hContent.AppendLine($"        m_owner.{typeOne.m_name}T::{field.m_name} = param;");
                    ...
                    hContent.AppendLine($"    template<typename T>");
                    hContent.AppendLine($"    auto {typeOne.m_name}Mem<T>::{SC.SnakeToPascalOrCamel(field.m_name)}::Get() const");
                    ...
                    hContent.AppendLine($"    template<typename T>");
                    hContent.AppendLine($"    std::string {typeOne.m_name}Mem<T>::{SC.SnakeToPascalOrCamel(field.m_name)}::GetName() const");
                    hContent.AppendLine(@"    {");
                    ...
                    hContent.AppendLine($"    template<typename T>");
                    hContent.AppendLine($"    std::string {typeOne.m_name}Mem<T>::{SC.SnakeToPascalOrCamel(field.m_name)}::GetValueString() const");
                    hContent.AppendLine(@"    {");
                    ...
                    hContent.AppendLine($"    template<typename T>");
                    hContent.AppendLine($"    void {typeOne.m_name}Mem<T>::{SC.SnakeToPascalOrCamel(field.m_name)}::Bind()");
                    hContent.AppendLine(@"    {");
                    hContent.AppendLine($"        m_state = FieldState::Bound;");
                    hContent.AppendLine(@"    };");
                    hContent.AppendLine();
                    ...
                    hContent.AppendLine($"    template<typename T>");
                    hContent.AppendLine($"    bool {typeOne.m_name}Mem<T>::{SC.SnakeToPascalOrCamel(field.m_name)}::IsBound() const");
                    hContent.AppendLine(@"    {");
                    ...
                    hContent.AppendLine($"    template<typename T>");
                    hContent.AppendLine($"    bool {typeOne.m_name}Mem<T>::{SC.SnakeToPascalOrCamel(field.m_name)}::IsKey() const");
                    hContent.AppendLine(@"    {");
                    ...
                    hContent.AppendLine($"    template<typename T>");
                    hContent.AppendLine($"    {typeOne.m_name}Mem<T>::{SC.SnakeToPascalOrCamel(field.m_name)}& {typeOne.m_name}Mem<T>::Get{SC.SnakeToPascalOrCamel(field.m_name)}()");
                    hContent.AppendLine(@"    {");
                    hContent.AppendLine($"        return *static_cast<{typeOne.m_name}Mem::{SC.SnakeToPascalOrCamel(field.m_name)}*>(m_p{SC.SnakeToPascalOrCamel(field.m_name)});");
                    hContent.AppendLine(@"    };");
                    hContent.AppendLine();
                }

                hContent.AppendLine($"    template<typename T>");
                hContent.AppendLine($"    auto {typeOne.m_name}Mem<T>::GetObjectName() const -> std::string");
                hContent.AppendLine(@"    {");
                ...
                hContent.AppendLine($"    template<typename T>");
                hContent.AppendLine($"    auto {typeOne.m_name}Mem<T>::GetFieldNames() const -> const std::vector<std::string>&");
                hContent.AppendLine(@"    {");
                ...
                hContent.AppendLine($"    template<typename T>");
                hContent.AppendLine($"    auto {typeOne.m_name}Mem<T>::GetFieldName(const int32_t fieldEnumValue) const -> std::string");
                hContent.AppendLine(@"    {");
                ...
                hContent.AppendLine($"    template<typename T>");
                hContent.AppendLine($"    auto {typeOne.m_name}Mem<T>::GetField(const std::string& fieldName) const -> const MemField*");
                hContent.AppendLine(@"    {");
                ...
                hContent.AppendLine($"    template<typename T>");
                hContent.AppendLine($"    auto {typeOne.m_name}Mem<T>::GetField(const int32_t fieldEnumValue) const -> const MemField*");
                hContent.AppendLine(@"    {");
                ...
                hContent.AppendLine($"    template<typename T>");
                hContent.AppendLine($"    auto {typeOne.m_name}Mem<T>::GetFields() const -> const std::vector<const MemField*>&");
                hContent.AppendLine(@"    {");
